...
- LittleFS 上の PNG 表情画像を各デバイスの画面サイズで表示
...
- `src/`: ファームウェア本体
- `data/`: CoreS3 用 LittleFS 表情画像。デフォルトの PNG を同梱しています
...
4. 後述の PNG ファイルを対象デバイスの画像ディレクトリに配置します。
...
このリポジトリには、`tools/face_image_builder/` のサンプル01から生成したデフォルトPNGを同梱しています。
上表の「つくよみちゃん万能立ち絵素材を使う場合の例」は、手動で画像を用意する場合の参考です。
つくよみちゃん素材そのものはこのリポジトリには含めていません。利用する場合は公式配布ページから素材を入手し、
利用規約に従って対象デバイスの推奨サイズで PNG を書き出し、対象デバイスの画像ディレクトリに配置してください。
...
- Wi-Fi が未設定の場合は、対象デバイスの SoftAP SSID に接続して
  ...
  設定することもできます。
- 表情が表示されない場合は、必要な PNG が対象デバイスの画像ディレクトリに揃っていることを確認し、
