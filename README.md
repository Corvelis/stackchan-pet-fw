...
- `good_*`、`bad_*`、`photo_*`などの旧静止顔を配布画像から削除し、既存端末向けの限定fallbackだけを残しました。
- CoreS3の640×480カメラ撮影と、サーボ・マイク・音声対話の安定性を改善しました。
- StopWatchに歩数カウンター、午前4時区切りの30日履歴、好感度報酬、アプリ同期を追加しました。
- WebSocket／USB Serial共通の発話吹き出しprotocolと状態診断を追加しました。
- 基本顔／なでなで用4×4スプライトシートの作成・分割・変換・検証toolを追加しました。
