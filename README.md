...
- `src/`: ファームウェア本体
- `data/`: LittleFS にアップロードする表情画像置き場。デフォルトのPNGを同梱しています
...
`data/` にはデフォルトの 240 x 240 PNG 表情画像を同梱しています。
自分の画像に差し替える場合は、次のファイル名で `data/` に置いてください。

画像生成AIで 6x6 のスプライトシートを作って表情画像を生成する場合は、
[`tools/face_image_builder/`](tools/face_image_builder/) を使えます。プロンプト、グリッドテンプレート、
スプライトシート分割CLI、`data/` への配置手順をまとめています。
