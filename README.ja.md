...
- WebSocket クライアント接続中は、マイク表示の上にあるカメラ表示をタップすると
  `camera_button` イベントを送ります。本体は WebSocket で画像を送らないため、
  クライアント側で HTTP `POST /capture` を呼びます。
...
します。`camera_button` は WebSocket クライアント接続中だけ送信し、phase は
`pressed`、クライアントから次の WebSocket text/binary 応答を受けるか 30 秒
