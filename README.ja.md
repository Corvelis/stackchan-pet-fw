...
- なでなで、ふりふり、接続開始、好感度レベル変化の本体側 interaction event
...
- 設定画面には Network / Display / Audio / Power のタブがあります。
- Network 設定ページを長押しすると、STA と SoftAP を切り替えます。
- Display 設定では、明るさ、画面オフ、低電力モードを操作できます。
- Audio 設定では、スピーカー音量を操作できます。
- Power 設定では、熱状態、バッテリー状態、低電力モードの提案を確認できます。
- 電源ボタンを押すと、画面のオン/オフを切り替えます。
- WebSocket クライアント接続中は、表情画面右側のマイク表示をタップすると
...
text frame は UTF-8 JSON コマンドです。binary frame は signed 16-bit PCM 音声です。
クライアントから本体へ送る binary frame はスピーカー再生用として扱い、本体は
listening 状態かつマイク送信がミュートされていない間、マイク音声を binary frame で
クライアントへ送ります。マイクのミュートは送信だけを止め、WebSocket 接続は切断しません。
...
本体は `petting`, `shake`, `session_start`, `level_up`, `level_down` などの
物理/本体側イベントを `interaction.event` として broadcast します。
