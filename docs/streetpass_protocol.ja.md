...
本体内部では Unix time（UTC）を保持し、タイムゾーンは日本時間
`Asia/Tokyo`（UTC+9）を基準にします。StopWatch の時計表示と歩数の日付区切りは
日本時間です。
...
Wi-Fi が接続されるたびに NTP 同期を開始し、NTP サーバーから応答を受けたことを
確認してから時刻を採用します。RTC とのずれが大きい場合も段階補正せず即時に
合わせ、その後は接続中に一定間隔で補正します。

外部アプリの `streetpass.time.set` もずれの大きさにかかわらず即時反映し、
システム時刻と RTC の両方へ書き戻します。アプリは接続時に現在の Unix time を
送信してください。
...
ファームウェアは `timezone` を `Asia/Tokyo` として扱います。履歴の別件判定は
UTC の経過秒数で行います。
