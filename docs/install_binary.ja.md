...
Release から次のファイルをダウンロードしてください。

```text
stackchan_cores3_factory.bin
```

`stackchan_cores3_factory.bin` は、bootloader、partition table、firmware、LittleFS 画像データを結合した
M5Stack CoreS3 向け factory image です。
...
このファームウェアでは、フリー素材キャラクター「つくよみちゃん」と、つくよみちゃん万能立ち絵素材を使用しています。
