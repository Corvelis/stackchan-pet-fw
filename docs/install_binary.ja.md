...
- 低電力モードをオンにすると、画面の最大輝度を抑え、待機中の表情更新を減らします。
  発話中の音声再生と口パクは継続します。
- CoreS3は右側、StopWatchは左下外周のマイク表示タップ、AtomS3R Chatbotは通常画面のBtnAダブルクリックで、マイク送信のミュート/解除を操作できます。
...
- 対応スマホアプリが撮影準備を通知している間は、通常画面右下のカメラ表示を短押しすると
  スマホへ撮影を要求し、約0.8秒長押しするとイン／アウトカメラの切替を要求します。詳しくは
