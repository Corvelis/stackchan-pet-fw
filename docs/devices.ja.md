...
- CoreS3とStopWatchは会話、ぐるぐる、タイムキーパー、旅モードに対応します。AtomS3Rは従来の会話とぐるぐる操作だけを使用します。詳しい操作は[体験モードと本体操作](experience_modes.ja.md)を参照してください。
...
| --- | --- | --- | --- |
| CoreS3 + ｽﾀｯｸﾁｬﾝ | 電源短押しで画面ON/OFF。電源ダブルクリックで会話／ぐるぐる切替。背面タッチでなでなで。左端から右フリックで4モード選択、右端から左フリックで設定画面 | 電源ダブルクリックで会話へ戻る。電源3クリック以上でタッチ/IMU切替。タッチ入力時は画面タッチ/ドラッグで16方向+中央。IMU入力時は本体傾き、画面長押しで基準リセット | Network 長押しでSTA/SoftAP切替。Servo画面で原点保存/復帰 |
| StopWatch | BtnA短押しで設定画面。BtnA約0.7秒長押しで4モード選択。BtnB/電源短押しで画面ON/OFF。画面中央タッチ/ドラッグでなでなで。接続中は左下のマイク表示タップでミュート。対応アプリが撮影準備済みのときは右下のカメラ表示を短押しでスマホ撮影、約0.8秒長押しでイン／アウト切替 | BtnA約0.7秒長押しでモード選択。BtnBダブルクリックでタッチ/IMU切替。BtnB長押しで基準リセット。タッチ入力時は画面タッチ/ドラッグで8方向+中央。IMU入力時は本体傾き | `<` / `>` または左右フリックでページ移動。Network長押しでSTA/SoftAP切替 |
...
タイムキーパーと旅モードの端末別操作は[体験モードと本体操作](experience_modes.ja.md)を参照してください。
...
- 左端から右へフリック: 4体験モードの選択画面を開く。設定画面表示中は設定を閉じる。
- 右端から左へフリック: 設定画面を開く。モード選択画面表示中は選択画面を閉じる。
- Network 画面を長押し: STA / SoftAP の切り替え。
- 電源ボタン短押し: 画面オン/オフ。
- 会話モードの背面タッチ: なでなで反応。タイムキーパーでは開始／一時停止／再開、旅モードでは表情ピッカーの表示／非表示。
- WebSocket または USB Serial クライアント接続中のマイク表示タップ: マイク送信ミュート/解除。
- カメラ表示タップ: `camera_button` イベント送信。
- Servo 画面: サーボ原点の保存と復帰。
- 電源ボタンダブルクリック: 会話モードとぐるぐるモードを切り替え。
...
- BtnA 短押し: 会話／ぐるぐるでは設定画面の表示/非表示。タイムキーパーでは開始／一時停止／再開。旅モードでは表情ピッカーの表示/非表示。
- BtnA 約0.7秒長押し: 4体験モードの選択画面を開く。画面内で選び、BtnAを離して閉じる。
- 旅モードでBtnAを2回以上: 通常顔へ戻す。
- BtnB 短押し、または電源ボタン短押し: 画面オン/オフ。
- タイムキーパーでBtnB短押し: ストップウォッチ計測中はラップ、一時停止／終了後はリセット。タイムキーパー中のBtnBは画面OFFに使いません。
...
- WebSocket または USB Serial クライアント接続中に通常画面左下外周のマイク表示をタップ: マイク送信のミュート/解除。マイク表示のタップ領域はなでなで判定から除外されます。
...
サーボ原点調整、本体カメラ撮影は使えません。対応スマホアプリが
`phone_camera.state`で準備完了と現在のレンズを通知すると、右下のカメラ表示から
スマホのリモート撮影を要求できます。表示中の`IN`／`OUT`で現在のレンズを確認でき、
約0.8秒長押しで切り替えられます。詳しくは
