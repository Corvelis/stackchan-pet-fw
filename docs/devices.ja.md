...
| StopWatch | BtnA短押しで設定画面。BtnAダブルクリックでぐるぐるON。BtnB/電源短押しで画面ON/OFF。画面中央タッチ/ドラッグでなでなで。接続中は左下のマイク表示タップでミュート。対応アプリが撮影準備済みのときは右下のカメラ表示を短押しでスマホ撮影、約0.8秒長押しでイン／アウト切替 | BtnAダブルクリックでぐるぐるOFF。BtnBダブルクリックでタッチ/IMU切替。BtnB長押しで基準リセット。タッチ入力時は画面タッチ/ドラッグで8方向+中央。IMU入力時は本体傾き | `<` / `>` または左右フリックでページ移動。Network長押しでSTA/SoftAP切替 |
